...
def find_closest_enemy(enemies, player_x, bullets):
    closest_enemy = None
    ...
    for enemy in enemies:
        targeted = False
        for bullet in bullets:
            if (bullet[0] == player_x + player_width // 2) and (bullet[1] > enemy[1]):
                targeted = True
                break
...
while not game_over:
    ...
    # AI logic
    closest_enemy = find_closest_enemy(enemies, player_x, bullets)
    if closest_enemy:
        ...
        # Shoot if aligned with the closest enemy and not already targeted by another bullet
        if abs(closest_enemy[0] + enemy_width // 2 - (player_x + player_width // 2)) < player_speed:
            if not any((bullet[0] == player_x + player_width // 2) and (bullet[1] > closest_enemy[1]) for bullet in bullets):
