...
# Initialize the video writer with debugging info
...
# Function to draw '80s style enemy ships
...
def draw_enemy_bullets(img, bullets):
    for bullet in bullets:
        cv2.circle(img, (bullet[0], bullet[1]), enemy_bullet_size, bullet_color, -1)
...
for frame_idx in range(frame_count):
    ...
    # Draw enemy bullets
    draw_enemy_bullets(frame, enemy_bullets)
    ...
    for bullet in bullets:
        ...
        if bullet[1] > 0:
            ...
            if hit_enemy:
                enemies_to_remove.append(hit_enemy)  # Mark the enemy for removal
            else:
                new_bullets.append(bullet)  # Keep the bullet if it didn't hit anything

    bullets = new_bullets

    # Remove the hit enemies from the `enemies` list
    enemies = [enemy for enemy in enemies if enemy not in enemies_to_remove]

    # Ensure that targeted enemies are still valid (not removed)
    targeted_enemies = [enemy for enemy in targeted_enemies if enemy in enemies]
    ...
    # Evasion logic
    for bullet in enemy_bullets:
        if abs(bullet[0] - (player_x + player_width // 2)) < player_speed * 2:
            if bullet[0] < player_x + player_width // 2 and player_x + player_width < width:
                player_x += player_speed
            elif bullet[0] > player_x + player_width // 2 and player_x > 0:
                player_x -= player_speed
    ...
    # Resize the frame to 3840x2160 to create the retro effect
    resized_frame = cv2.resize(frame, (1920, 1080), interpolation=cv2.INTER_NEAREST)

    # Write the frame to the video
    print(f"Writing frame {frame_idx + 1} to video.")
    video_writer.write(resized_frame)

    # Add a small delay to ensure the video writer flushes its buffer
    cv2.waitKey(1)
    ...
    for enemy in enemies:
        if random.randint(1, 100) == 1:  # Random chance of shooting
